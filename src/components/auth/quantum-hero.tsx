        ...
        <FloatingCard className="absolute right-0 top-12" delay={0.25}>
          <div className="flex items-center gap-2">
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[color:var(--accent-soft)]">
              <svg viewBox="0 0 24 24" className="h-3 w-3" fill="none" stroke="#6D5AF0" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
    ...
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <svg className="absolute inset-0 h-full w-full opacity-[0.45]" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="qgrid" width="36" height="36" patternUnits="userSpaceOnUse">
            <path d="M36 0H0V36" fill="none" stroke="#0A0A0A" strokeOpacity="0.08" strokeWidth="0.5" />
          </pattern>
      ...
      <svg className="absolute inset-0 h-full w-full" preserveAspectRatio="none" viewBox="0 0 100 100">
        <motion.line
          ...
          stroke="url(#beam1)" strokeWidth="0.2"
          initial={{ pathLength: 0, opacity: 0 }}
          animate={{ pathLength: [0, 1, 1], opacity: [0, 0.7, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.line
          x1="90" y1="15" x2="15" y2="85"
          stroke="url(#beam2)" strokeWidth="0.2"
          initial={{ pathLength: 0, opacity: 0 }}
...
function ChipBlueprint() {
  ...
  return (
    <svg
      ...
      aria-hidden
    >
      <rect x="40" y="30" width="240" height="180" rx="18" fill="white" stroke="#0A0A0A" strokeOpacity="0.5" />
      ...
      <text x="160" y="123" textAnchor="middle" fontSize="8" fontFamily="ui-monospace, monospace" fill="#0A0A0A">QPU</text>
    </svg>
  );
}