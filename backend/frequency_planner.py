...
class FrequencyPlanner:
    ...
    def _assign_qubit_frequencies(self) -> List[QubitSpec]:
        ...
        For grid/heavy_hex: uses (row + col) % 2 checkerboard.
        For line: simple alternation by index.
        For ring/star: alternation by index.
        """
        import math, random
        ...
        # Determine group for each qubit index (0-based)
        # Group A = even parity, Group B = odd parity
        # For a sqrt(n) × sqrt(n) grid, parity = (row + col) % 2
    ...
    def _assign_resonators(
        ...
        for i, q in enumerate(qubits):
            f_r    = round(RESONATOR_BASE_GHz + i * RESONATOR_STEP_GHz, 4)
